...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	var event Event
	if err := db.QueryRow("SELECT id, title, public_fg, closed_fg, price FROM events WHERE id = ?", eventID).Scan(&event.ID, &event.Title, &event.PublicFg, &event.ClosedFg, &event.Price); err != nil {
		return nil, err
	}
	...
	r, _ := db.Query("SELECT * FROM reservations where event_id = ? AND canceled_at IS NULL ORDER BY reserved_at;", event.ID)
	for r.Next() {
		var reservation Reservation
		r.Scan(&reservation.ID, &reservation.EventID, &reservation.SheetID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt)
